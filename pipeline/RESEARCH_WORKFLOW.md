...
- 매일 발행하거나 주간 건수를 채우지 않는다. 적합한 기사가 있을 때 비정기적으로 게시한다.
- 원문에 지역·기간·주택 유형·비교 대상·주장이 명확하고 같은 범위의 원자료를 확보할 수 있어야 한다.
- 상승과 하락 주장에 같은 비교·선정 기준을 적용한다.
- 50개 선정 단지는 보조 자료다. 경기도·서울·전국 전체의 주장을 대신 검증하지 않는다.
- 기사 전체의 진위나 기자의 의도를 판정하지 않는다.
- 공개 요약은 기사 주장 → 확인한 사실 → 해석의 한계 순서로 작성한다.
- 자동 선별은 최대 12개 후보를 탐색해 적합한 초안을 최대 3개 만든다. 0개도 정상이다.
- 현재 자동 범위 추출은 지역 아파트 거래량 방향을 지원한다. 가격 주장은 운영자가 범위를 확인한다.
- 공개는 운영자 승인 후에만 가능하다. 수집·분석은 현재 로컬 검증기, 게시 승인 기록은 운영 Supabase를 사용한다.

아래는 기존 구현 설명이며, 독자 요청 입력 대신 운영자 검토함에서 범위를 보완한다.
