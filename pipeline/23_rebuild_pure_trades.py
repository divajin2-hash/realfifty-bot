...
def run_rebuild():
    ...
    year_months = [f"{y}{m:02d}" for y in range(2016, 2027) for m in range(1, 13) if not (y == 2026 and m > 7)]

    print(f"Wiping existing rtms_transactions (containing direct trades)...")
    for c in complexes:
        supabase.table("rtms_transactions").delete().eq("complex_id", c["id"]).execute()

    print("Rebuilding PURE trades (No direct trades)...")
    ...
    for lawd, comps in lawd_map.items():
        ...
        # Rights only for recent years to save time (2020+)
        recent_ymds = [ymd for ymd in year_months if int(ymd[:4]) >= 2020]
