...
def evidence(d):
 facts = {'gap':{'label':'평균 호가 괴리 (%)','value':d['meanGap'],'n':len(d['comparable'])},'drop':{'label':'관측 고점 대비 최근 실거래 평균 (%)','value':d['meanDrop'],'n':len(d['comparable'])},'below':{'label':'실거래 아래 호가 단지 수','value':d['below'],'n':len(d['comparable'])},'coverage':{'label':'호가 수집 타입 수','value':d['coveredTypes'],'n':d['typeCount']},'monthly':{'label':'최근 월 신고 집계 (미완결 가능)','value':d['latest']},'complete_month':{'label':'직전 완료 월 집계 (신고 지연 가능)','value':d['lastComplete']}}
 try:
  daily=json.loads((DATA/'daily_changes.json').read_text(encoding='utf-8'))
  if not daily.get('unavailable') and datetime.fromisoformat(daily['generatedAt'])==datetime.fromisoformat(d['updatedAt']):
   facts['daily_asks']={'label':'타입별 최저호가의 수집분 비교 (동일 매물 가격 수정 아님)','date':daily['date'],'previous_date':daily['previousDate'],'comparable':daily['comparable'],'rises':daily['rises'],'falls':daily['falls'],'unchanged':daily['unchanged']}
