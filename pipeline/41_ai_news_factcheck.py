...
def run():
    ...
    factcheck_prompt = f"""
    너는 가짜뉴스/과장보도를 판별하는 'RealFifty 팩트체커'입니다.
    제공된 [오늘의 뉴스]들을 모두 읽고, 내용이 매우 유사하거나 복붙(어뷰징)된 기사들이 있다면 병합하여 가장 대표적인 기사 1~2개만 엄선해 주세요.
    그리고 그 엄선된 1~2개의 최고 핵심 기사에 대해서만 [RealFifty 데이터]를 철저한 근거로 삼아 과장인지 사실인지 팩트체크해 주시기 바랍니다. 말투는 유쾌하고 존댓말을 사용하세요.
    결과는 반드시 1개 이상 2개 이하의 원소를 가진 완벽한 JSON 배열 형태로만 출력해야 합니다. 마크다운 백틱 안됨(```json ... ``` 안됨). 시작과 끝이 [ 와 ] 이어야 합니다.
    ...
    [
      {{
        ...
        "factcheck_content": "RealFifty 분석결과: 데이터에 따르면 평균 하락률이 어쩌고... 하면서 반박하거나 동의하는 5~6줄 깊이있는 분석 텍스트 (존댓말 사용, 단호하지만 위트있게)"
      }}
    ]
    * 절대 제공받은 뉴스와 똑같은 개수를 만들지 말고, 중복을 확실히 제거하여 1~2개의 핵심 기사 팩트체크 오브젝트만 배출하세요 (배열 원소 개수 최대 2개).
    """
