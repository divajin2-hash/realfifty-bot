...
export default async function ReportPage({ searchParams }: {
    ...
    const r = await readReport(requested);
    if (requested && r.date && requested !== r.date) redirect(`/report?date=${r.date}`);
    const d = r.market;
    ...
    const daily = r.document?.evidence?.daily_asks;
    if (daily) evidence.daily_asks = `${daily.previous_date} → ${daily.date} · 비교 ${daily.comparable}개 평형 · 호가 상승 ${daily.rises}개 / 하락 ${daily.falls}개 / 동일 ${daily.unchanged}개`;
 ...
 <section className="rt-panel"><div className="rf-eyebrow">ANALYST COMMENTARY</div><h2>{report ? 'AI 심층 해석' : '지표 해석 안내'}</h2>{report ? report.analysis.map((s, i) => <div className="rt-scenario" key={i}><h3>{s.title}</h3><p>{s.body}</p><p className="rt-small">근거: {s.evidence_ids.map(id => evidence[id] || id).join(' · ')}</p></div>) : <><p>호가 괴리는 매도자의 기대 가격과 마지막 체결가의 간격입니다. 거래 후 시간이 오래 지났거나 층·향·상태가 다르면 차이가 커질 수 있습니다. 단지 상세에서 거래일과 매물 조건을 먼저 확인해야 합니다.</p><p>월 신고 건수는 거래 활력을 관찰하는 출발점입니다. 이번 달은 신고가 진행 중이므로 지난달 전체와 직접 비교한 감소율은 제공하지 않습니다. 충분한 신고 기간이 지난 뒤 같은 길이의 기간으로 다시 비교해야 합니다.</p></>}</section>
